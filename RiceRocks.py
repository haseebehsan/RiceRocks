...
def start_game():
    global score, lives, time, my_ship, rock_group, missile_group, explosion_group, started, timer
...
def stop_game():
    global rock_group, started, timer
...
nebula_image = simplegui.load_image("https://dl.dropboxusercontent.com/u/96572061/public%20photos/nebula_blue.f2014.png")
...
class Sprite:
    ...
    def collide(self, other_object):
        ...
        real_dist = dist(self.pos, other_object.get_position())
    ...
    def draw(self, canvas):
        if self.animated:
            image_center = [0, 0]
            image_center[0] = self.image_center[1] + ((self.age // 3) % 64) * self.image_size[0]
    ...
    def update(self):
        ...
        # return if we want to keep the sprite
...
def group_collide(group, other_object):
    ...
    for an_object in group:
        if an_object.collide(other_object):
            group.discard(an_object)
            explosion_group.add(Sprite(an_object.get_position(), [0, 0], 0, 0, explosion_image, explosion_info))
...
def group_group_collide(group1, group2):
    ...
    for an_object in group1:
        noc = group_collide(group2, an_object)
        if noc > 0:
            num_of_collides += noc
            group1.discard(an_object)
    return num_of_collides
...
#    canvas.draw_image(debris_image, [size[0]-wtime + 1, center[1]], [2*wtime + 1, size[1]], 
#                                [1.25*wtime + 1, height/2], [2.5*wtime + 1, height])
...
def rock_spawner():
    ...
    if len(rock_group) < max_rocks and started:
        ...
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image[random.randrange(3)], asteroid_info)
